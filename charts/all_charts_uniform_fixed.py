...
ax.text(0,  0.10, f"{total:,}", ha="center", va="center",
        color=DARK_GREEN, **FP(72))          # was 54

ax.text(0, -0.16, "census tracts", ha="center", va="center",
        color=GRAY, **FP(FS_STATS + 30))     # was FS_STATS
...
for i, (wedge, lbl, col) in enumerate(
    ...
    ax.annotate(lbl,
        ...
        linespacing=1.4, **FP(FS_BAR_LBL + 40))

ax.axis("equal")
# REPLACE WITH:
save(fig, "chart3_indicator_weight_pie.png")
# ════════════════════════════════════════════════════════════════
...
# Build top-10 from county-level data
top10 = (county
         .sort_values("Healthcare_Desert_Score", ascending=False)
         .head(10)
         .reset_index(drop=True))

# Bar colours — red for CRITICAL, orange for HIGH
bar_colors = [
    RED if t == "CRITICAL" else ORANGE
    for t in top10["Risk_Tier"]
]

# Legend patches for Chart 4
leg_patches = [
    mpatches.Patch(color=RED,    label="CRITICAL (≥ 22.0)"),
    mpatches.Patch(color=ORANGE, label="HIGH (19–21.9)"),
]
...
# Legend — upper-left, nudged right so it clears the y-axis labels
legend = ax.legend(handles=leg_patches,
                   loc="upper right",
...
ax.set_xlabel("Social Determinants of Health (SDoH) Indicators", 
              color=GRAY,
              labelpad=60)
ax.set_ylim(0, max(high_v) * 1.40)
...
fig.tight_layout(pad=2.0, rect=[0, 0.08, 1, 1])  # bottom margin added
